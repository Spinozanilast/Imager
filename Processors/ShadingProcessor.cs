...
{
    public class ShadingProcessor
    {
        private readonly int[,] _grayScaleMatrix;

        public ShadingProcessor(int[,] grayScaleMatrix)
        {
            _grayScaleMatrix = grayScaleMatrix ?? throw new ArgumentNullException(nameof(grayScaleMatrix));
        }

        public int[,] CalculateManhattanDistance()
        {
            int width = _grayScaleMatrix.GetLength(0) + 2;
            int height = _grayScaleMatrix.GetLength(1) + 2;
        ...
        private void CalculateDistances(int[,] matrix, int[,] blackDistances, int[,] whiteDistances, int maxDistance)
        {
            ...
            for (int i = 1; i < width - 1; i++)
            {
                for (int j = 1; j < height - 1; j++)
                {
                    ...
                    int distance = isBlack ? 0 : maxDistance;
